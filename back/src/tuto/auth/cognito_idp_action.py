...
class CognitoIdentityProviderWrapper:
    ...
    def respond_to_email_otp_challenge(self, user_name, session, email_otp_code):
        """
        Responds to a challenge for an EMAIL_OTP. This completes the second step of
        a two-factor sign-in. When sign-in is successful, it returns an access token
        that can be used to get AWS credentials from Amazon Cognito.
        ...
        :param session: Session information returned from a previous call to initiate
                        authentication.
        :param email_otp_code: A code that your user pool delivered in an email message.
        :return: The result of the authentication. When successful, this contains an
                 access token for the user.
        """
        try:
    ...
    def admin_set_user_password(self, user_name: str, password: str, permanent: bool):
        """
        Sets the specified user’s password in a user pool.This operation administratively
        sets a temporary or permanent password for a user.

        :param user_name: The name of the user who is signing in.
        :param password: The new password to set for the user.
        :param permanent: Set to true to set a password that the user can immediately sign in with.
                          Set to false to set a temporary password that the user must change on their next sign-in.
        """
        try:
            kwargs = {
                "UserPoolId": self.user_pool_id,
            ...
            self.cognito_idp_client.admin_set_user_password(**kwargs)
        except ClientError as err:
            error_code: str = err.response["Error"]["Code"]
            error_message: str = err.response["Error"]["Message"]
            logger.error(
                f"Couldn't change password for {user_name}. Here's why: {error_code}: {error_message}"
            )
            raise
        else:
            return True
    ...
    def respond_to_new_password_challenge(
        self, user_name, session, new_password, user_attributes=None
    ):
        """
        Responds to a NEW_PASSWORD_REQUIRED challenge. This occurs when a user is required
        to set a new password, typically after an admin sets a temporary password.

        :param user_name: The name of the user who is signing in.
        :param session: Session information returned from a previous call to initiate
                        authentication.
        :param new_password: The new password to set for the user.
        :param user_attributes: Optional user attributes to update during password change.
        :return: The result of the authentication. When successful, this contains an
                access token for the user.
        """
        try:
            ...
            if user_attributes:
                for attr_name, attr_value in user_attributes.items():
                    challenge_responses[f"userAttributes.{attr_name}"] = attr_value
            ...
            logger.error(
                "Couldn't respond to new password challenge for %s. Here's why: %s: %s",
                user_name,
                err.response["Error"]["Code"],
                err.response["Error"]["Message"],
            )
    ...
    def admin_reset_user_password(self, user_name):
        """
        Initiates a password reset for a user using admin privileges.
        This sends a password reset confirmation code to the user's registered email address.

        :param user_name: The name of the user who needs password reset.
        :return: Information about where the confirmation code was sent.
        """
        try:
            response = self.cognito_idp_client.admin_reset_user_password(
                UserPoolId=self.user_pool_id,
                Username=user_name,
            )
            # admin_reset_user_password doesn't return delivery details like forgot_password
            # but we can simulate the response for compatibility
            return {
            ...
            logger.error(
                "Couldn't reset password for %s. Here's why: %s: %s",
                user_name,
                err.response["Error"]["Code"],
                err.response["Error"]["Message"],
            )
    ...
    def confirm_forgot_password(self, user_name, confirmation_code, new_password):
        """
        Confirms a forgot password request and sets a new password for the user.

        :param user_name: The name of the user whose password is being reset.
        :param confirmation_code: The confirmation code sent to the user's email.
        :param new_password: The new password to set for the user.
        :return: True when the password reset succeeds.
        """
        try:
            kwargs = {
                "ClientId": self.client_id,
    ...
    def admin_create_user(
        self, user_name, email, temporary_password, send_invitation=False
    ):
        """
        Creates a new user in the user pool using admin privileges.
        The user will receive a temporary password and must change it on first sign-in.

        :param user_name: The username for the new user.
        :param email: The email address for the new user.
        :param temporary_password: The temporary password for the new user.
        :param send_invitation: Whether to send an invitation message to the user.
        ...
        try:
            kwargs = {
                ...
                "MessageAction": "SUPPRESS" if not send_invitation else "DELIVER",
            }

            response = self.cognito_idp_client.admin_create_user(**kwargs)
        except ClientError as err:
            error_code = err.response["Error"]["Code"]
            error_message = err.response["Error"]["Message"]
            logger.error(
                "Couldn't create user %s. Here's why: %s: %s",
                user_name,
                error_code,
                error_message,
            )
    ...
    def admin_update_user_attributes(self, user_name, user_attributes):
        """
        Updates user attributes in the user pool using admin privileges.

        :param user_name: The username of the user to update.
        :param user_attributes: A dictionary of attribute names and values to update.
        ...
        try:
            ...
            self.cognito_idp_client.admin_update_user_attributes(**kwargs)
        except ClientError as err:
            error_code = err.response["Error"]["Code"]
            error_message = err.response["Error"]["Message"]
            logger.error(
                "Couldn't update user attributes for %s. Here's why: %s: %s",
                user_name,
                error_code,
                error_message,
            )
    ...
    def admin_delete_user(self, user_name):
        """
        Deletes a user from the user pool using admin privileges.

        :param user_name: The username of the user to delete.
        :return: True when the deletion succeeds.
        """
        try:
            kwargs = {"UserPoolId": self.user_pool_id, "Username": user_name}
